...
const saveDirectory = async ({ token, config, file, data, message }) => {
  ...
  if (!updateResponse.ok) {
    throw new Error(result?.message || `GitHub rejected the directory update (${updateResponse.status}).`);
  }
  return result;
};

const normalizeRecordType = (payload) => typeof payload?.record_type === "string"
  ? payload.record_type.trim().toLowerCase()
  : "";
...
const validateRecord = (record, config) => {
  ...
  const slug = typeof record[config.slugKey] === "string" ? record[config.slugKey].trim() : "";
  const name = typeof record[config.nameKey] === "string" ? record[config.nameKey].trim() : "";
  if (!slug) problems.push(`${config.slugKey} is required.`);
  else if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) problems.push(`${config.slugKey} must use lowercase letters, numbers, and single hyphens only.`);
  if (!name) problems.push(`${config.nameKey} is required.`);
  return problems;
};

const handleDelete = async ({ payload, env, recordType, config }) => {
  ...
  try {
    ...
    const result = await saveDirectory({
...
const handleUpdate = async ({ payload, env, recordType, config }) => {
  const record = payload?.record;
  const problems = validateRecord(record, config);
  if (problems.length) return jsonResponse({ error: "Record validation failed.", problems }, 400);

  const originalSlug = typeof payload.original_slug === "string" && payload.original_slug.trim()
    ? payload.original_slug.trim()
    : record[config.slugKey].trim();
  const newSlug = record[config.slugKey].trim();
  ...
  try {
    ...
    const existingIndex = records.findIndex((item) => item?.[config.slugKey] === originalSlug);
    if (existingIndex < 0) {
      return jsonResponse({ error: `${recordType === "event" ? "Event" : "Business"} slug was not found: ${originalSlug}` }, 404);
    }

    if (newSlug !== originalSlug) {
      const duplicateIndex = records.findIndex((item, index) => index !== existingIndex && item?.[config.slugKey] === newSlug);
      if (duplicateIndex >= 0) {
        return jsonResponse({ error: `Another ${recordType} already uses the slug ${newSlug}.` }, 409);
      }
    }

    records[existingIndex] = record;
    data[config.arrayKey] = records;
    ...
    const result = await saveDirectory({
      ...
      file,
      data,
      message: `Update ${recordType} ${newSlug} ${new Date().toISOString()}`
    });

    return jsonResponse({
      success: true,
      message: `${recordType === "event" ? "Event" : "Business"} ${record[config.nameKey]} was updated.`,
      slug: newSlug,
...
export const onRequestPost = async ({ request, env }) => {
  ...
  return handleUpdate({ payload, env, recordType, config });
};
